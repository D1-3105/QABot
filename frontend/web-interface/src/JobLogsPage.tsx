...
const JobLogsPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchLogs = async () => {
            // /// Generated by Qwen 3.8 27B
            //
            // Batch state updates: one setLogs() per line re-renders the whole
            // list and re-runs the stats/dedup memos, which is O(n^2) for large
            // logs. Coalesce to at most one state update per FLUSH_MS. A line
            // only waits if another line was flushed within FLUSH_MS, so the
            // first line of a tail burst (bursts are seconds apart) still
            // flushes immediately - live-tail latency is unchanged.
            const FLUSH_MS = 500;
            let pendingLogs: ParsedLogLine[] = [];
            let lastFlush = 0;
            const flushPending = (force = false) => {
                ...
                pendingLogs = [];
                setLogs(prevLogs => [...prevLogs, ...batch]);
            };
            const flushTimer = window.setInterval(() => flushPending(true), FLUSH_MS);
            //
            // \\\ Generated by Qwen 3.8 27B
